...
import { refFirebase, deleteFirebaseObject, storage } from '../../../../Firebase';
...
export default function BlogNewPostForm({ onCreateBlog, user }) {
  ...
  const handleDrop = useCallback(
    (acceptedFiles) => {
      setFileLoading(true);
      let newCover = values.cover;
      acceptedFiles.map(async (file) => {
        const fileRef = ref(storage, file.name);
        await uploadBytes(fileRef, file);
        const fileUrl = await getDownloadURL(fileRef);
        newCover += fileUrl;
        setFieldValue('cover', newCover);
      });
      setFileLoading(false);
    },
    [setFieldValue, values.cover]
  );
